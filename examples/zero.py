...
def do_mul(primitive, x : ImplicitZeros, y : jnp.ndarray):
...
def handle_add_general(primitive, x : ImplicitZeros, y : jnp.ndarray):
...
def main():
    ...
    def f(x, y):
        ...
        a = jnp.sum(w)           # a: jnp.ndarray    output: UserWarning: Primitive reduce_sum was not
                                 #                           handled by class ImplicitZeros, so implicit
                                 #                           args will be materialized
        b =  w + a               # b: jnp.ndarray    output: Invoked handle_add
        return b
